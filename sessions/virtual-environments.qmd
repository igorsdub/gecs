...
gitGraph
   ...
   checkout main
   merge set-up-venv id: "Merge PR: set-up-venv"
```
...
gitGraph
   ...
   checkout main
   merge add-word-counting id: "Merge PR: add-word-counting"
```
...
gitGraph
   ...
   checkout main
   merge separate-data id: "Merge PR: separate-data"
```
...
gitGraph
   ...
   checkout main
   merge create-package id: "Merge PR: create-package"
```
...
### Moving from Scripts to a Package

Before creating files, let's understand why we are refactoring `scripts/count_words.py` into a package module `src/bookstats/counts.py`:

| Feature | Standalone Script (`scripts/count_words.py`) | Package Module (`src/bookstats/counts.py`) | Architectural Benefit |
|---|---|---|---|
| **Interface** | Tightly coupled to command-line arguments (`sys.argv`) | Pure, decoupled functions (`extract_words`, `count_words`, `process_book_file`) | Reusable in notebooks, automated test suites (Session 3), and automation pipelines (Session 4). |
| **Documentation** | Minimal one-line docstrings | Structured NumPy-style docstrings (`Parameters`, `Returns`) | Enables autocomplete, type inference, and automated linting compliance. |
| **Data Flow** | Single-file processing only | Added `--combine` mode for aggregation | Merges multiple book count CSVs into a unified table (`data/processed/book-counts.csv`). |
...
::: {.callout-note title="What is a Package API and Why Does __init__.py Define It?"}
...
By defining `__all__` inside `src/bookstats/__init__.py`:
1. **Public vs. Internal Seam**: You explicitly declare which functions constitute your supported public API (`extract_words`, `count_words`, `combine_word_counts`).
2. **Simplified Imports**: Collaborators can simply write `from bookstats import count_words` without needing to know or care which internal module contains the logic.
3. **Stability Guarantee**: If you later reorganize internal files (e.g. splitting counting algorithms into separate submodules), external scripts, tests (Session 3), and Make workflows (Session 4) will continue to work without changing their import statements.

::: {.callout-tip title="Advanced: Lazy Imports" collapse="true"}
In very large scientific libraries (like SciPy or Polars), importing everything immediately in `__init__.py` can slow down startup times. Advanced packages sometimes use Python 3.7+ module-level `__getattr__` to dynamically load submodules only when accessed. For our package, explicit imports provide the clearest, most readable foundation.
:::
:::
...
Notice how our data flow is strictly decoupled:
1. `uv run python -m bookstats.counts <raw-book> <intermediate-csv>` runs headlessly from the command line to generate per-book counts.
2. `uv run python -m bookstats.counts --combine ... -o data/processed/book-counts.csv` aggregates them into an analysis-ready dataset.
3. The visualization notebook does **not** process raw text or duplicate counting logic—it reads the pre-computed `data/processed/book-counts.csv` table.

::: {.callout-note title="This analysis an automate would make"}
In research software engineering, interactive notebooks that embed heavy preprocessing often fail to reproduce because they require humans to click cells in arbitrary order.

By keeping `bookstats.counts` as an unassisted command-line engine, the analysis pipeline can be executed start-to-finish without human interaction. This is the exact analysis that an automation tool (such as **GNU Make** in Session 4) will orchestrate incrementally, and that **GitHub Actions** will execute on remote virtual machines to build and publish an interactive GitHub website!
:::

### Interactive Visualization with Marimo

Now that our package produces structured, aggregated count data, let's explore it interactively!
...
We set up and launch our interactive visualization in three simple steps:

#### Step 1: Install Marimo and Altair

Add `marimo` and `altair` to your project dependencies using `uv`:

```bash
uv add marimo altair
```

#### Step 2: Create `notebooks/visualize.py`
...
#### Step 3: Launch the Interactive Server

Start the Marimo notebook server from your terminal:

```bash
uv run marimo edit notebooks/visualize.py
```

Marimo will open an interactive interface in your default browser:
...
- In Session 3, we will extend this notebook using Test-Driven Development (TDD) to compute descriptive Zipf power-law fits and overlay regression lines.
