...
Before attending Session 1, verify that your computer meets the following four readiness gates:

| Gate | Requirement | Verification Check |
|---|---|---|
| **Gate 1: GitHub Account & 2FA** | Active GitHub account with 2FA or passkey enabled | Can sign in at [github.com](https://github.com) |
| **Gate 2: VS Code & Profile** | Visual Studio Code installed with the GECS profile loaded | VS Code opens and displays your environment |
| **Gate 3: Git Identity** | Git installed with your name and email configured | `git config user.name` outputs your identity |
| **Gate 4: Terminal & uv** | Working shell with `uv` package manager installed | `uv --version` returns version number |
...
# Step 1: GitHub Account & Authentication {#sec-github}
...
2. **Enable Multi-Factor Authentication (2FA)**: GitHub requires 2FA for all accounts. We recommend registering a **Passkey** (Touch ID, Face ID, or Windows Hello) via your GitHub **Settings > Password and authentication**. Alternatively, use an authenticator app (e.g., Google Authenticator, 1Password).
...
Configure Git with your name and the email address associated with your GitHub account:
...
Configure Git with your name and GitHub-associated email address:
...
In the VS Code WSL terminal, configure Git:
...
# Pre-Course Setup Clinic & Troubleshooting {#sec-troubleshooting}
...
- **BIOS Virtualization on Windows**: If WSL reports that virtualization is disabled, enter your PC BIOS/UEFI settings on startup and enable **Intel VT-x** or **AMD-V**.
