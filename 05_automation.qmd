...
## Starting from the current project
...
`make` reads instructions from a file called `Makefile`. Let us start by automating one real step from the project: generating a count table for Dracula.
...
.PHONY : all
all : $(FIGURE_FILES) ## Build figures for all books

.PHONY : clean
clean : ## Remove generated counts and figures
	rm -f counts/*.tsv figures/*.html
```
