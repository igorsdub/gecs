...
Another powerful command you encountered during [the setup process](00-setting-up.qmd) is `curl` (**C**lient **URL**). It's used to transfer data from or to a server, often for downloading files or scripts from the internet:
...
These files are executed every time you start a terminal. During the [setup process](00-setting-up.qmd), you modified these files when installing tools like Pixi and Starship. For example, you added lines like:
