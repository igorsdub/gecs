...
### Why uv?
 ...
 `uv` is an blazingly fast Python package and project manager. If you used Conda or Poetry before, you will feel the difference. It has a few advantages that make it a good fit for this course:
...
This creates the directory `gecs-uv` for you. If you already had a project folder and wanted to initialize it in place, you could first `cd` into it and then run:

``` bash
uv init .
```

One more nice detail: `uv init` also initializes a Git repository by default, so you do **not** need to run `git init` yourself afterward.
...
`uv` also created a `.python-version` file for us. That file pins the default Python version request for this project.
...
## Version-control the project from the start
...
``` bash
uv add marimo numpy matplotlib
```
...
git commit -m "Add marimo and plotting dependencies"
```
...
That is another reproducibility tool: one project manifest, one lock file, and one dependency graph that `uv` can recreate consistently across machines.
...
To explore marimo's built-in plotting tutorial:

``` bash
uv run marimo tutorial plots
```
...
For example, you could try marimo like this:

``` bash
uvx marimo tutorial intro
```

If you want an extra-isolated throwaway environment, you can also use:

``` bash
uvx --isolated marimo tutorial intro
```

This is handy when you just want to explore a tool, check whether you like it, or run something once without making your project environment bigger.
