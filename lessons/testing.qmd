...
This test follows the same path as the real workflow: book → count file → plot. It calls `plot_word_counts()`, so it requires Selenium and a locally available browser for PNG export.
