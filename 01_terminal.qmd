...
``` bash
echo "VISION, CHALLENGES, INNOVATION" > syndey_brenner.statue
cat syndey_brenner.statue
```
...
``` bash
cat syndey_brenner.statue svante_paabo.statue
```
...
You can confirm that the job has been done by using `ls` to see that it’s missing from the working directory, then `cd dir1` to change into ***lab1***, `ls` to see that it’s in there, then `cd ..` to move the working directory back again. Or you could save a lot of typing by passing a path directly to the `ls` command to get straight to the confirmation you’re looking for:
